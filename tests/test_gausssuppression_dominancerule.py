...
num = np.array(
    [100, 90, 10, 80, 20, 70, 30, 80, 10, 10, 70, 10, 10, 10, 60, 20, 10, 10]
)
v1 = np.concatenate(
    (
        np.array("v1")[np.newaxis],
        np.repeat(["v2", "v3", "v4"], 2),
        np.repeat("v5", 3),
        np.repeat(["v6", "v7"], 4),
    )
)
sw2 = np.array([1, 2, 1, 2, 1, 2, 1, 2, 1, 1, 2, 1, 1, 1, 2, 1, 1, 1])
sw3 = np.array([1, 0.9, 1, 2, 1, 2, 1, 2, 1, 1, 2, 1, 1, 1, 2, 1, 1, 1])

d = pd.DataFrame({"v1": v1, "num": num, "sw1": 1, "sw2": sw2, "sw3": sw3})


mm = SSBtools.ModelMatrix(d, formula="~ v1 - 1", crossTable=True, sparse=True)
...
def test_Default_weighted_dominance():
    ...
    assert all(
        np.concatenate((np.array(True)[np.newaxis], np.repeat([False], 6)))
        == p["primary"]
    )
...
def test_tauargus_dominance():
    ...
    assert np.all(
        p["primary"] == np.array([True, True, False, False, False, False, False])
    )
