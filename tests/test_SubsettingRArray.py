...
def test_SubsettingRArray():
    ...
    arr = bs.matrix(np.arange(12)+1, nrow=4, ncol=3,
                    dimnames=bs.list(bs.c("a", "b", "c", "d"),
                                     bs.c("A", "B", "C")))
    arr.toR()
    arr[0:2, 0:2].toR()
    ...
    assert np.all(arr[np.sum(arr, axis=1) > 20, :]._Rattributes["dimnames"][0] == ["c", "d"])
    assert np.all(arr[np.sum(arr, axis=1) > 20, :]._Rattributes["dimnames"][1] == ["A", "B", "C"])
